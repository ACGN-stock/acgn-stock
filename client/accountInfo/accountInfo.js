...
Template.accountInfoLog.helpers({
  getLogDescriptionHtml(logData) {
    switch (logData.logType) {
      ...
      case '交易紀錄': {
        if (logData.username[0] === Meteor.user().username) {
          return '以$' + logData.price + '的單價向' + (logData.username[1] || ('「' + generateCompanyLink(logData.companyName) + '」公司')) + '購買了' + logData.amount + '數量的「' + logData.companyName + '」公司股票！';
        }
        else {
      ...
      case '辭職紀錄': {
        return '辭去了「' + generateCompanyLink(logData.companyName) + '」公司的經理人職務。';
      }
      case '參選紀錄': {
        return '參與了「' + generateCompanyLink(logData.companyName) + '」公司在該商業季度的經理人競選活動。';
      }
      case '經理管理': {
        return '以經理人的身份修改了「' + generateCompanyLink(logData.companyName) + '」公司的一些資訊。';
      }
      case '產品發布': {
        return '以經理人的身份為「' + generateCompanyLink(logData.companyName) + '」公司發表了一項新產品。';
      }
      case '產品下架': {
        return '以經理人的身份將「' + generateCompanyLink(logData.companyName) + '」公司的一項產品給下架了。';
      }
      case '推薦產品': {
        return '向「' + generateCompanyLink(logData.companyName) + '」公司的一項產品投了一張推薦票。';
      }
      case '支持紀錄': {
      ...
      case '舉報公司': {
        return '以「' + logData.message + '」理由舉報了「' + generateCompanyLink(logData.companyName) + '」公司。';
      }
      case '舉報產品': {
        return '以「' + logData.message + '」理由舉報了「' + generateCompanyLink(logData.companyName) + '」公司的一項產品。';
      }
      case '公司撤銷': {
...
function generateCompanyLink(companyName) {
  const href = FlowRouter.path('company', {companyName});

  return '<a href="' + href + '" target="_blank">' + companyName + '</a>';
}
